...
def main(image_data, epochs):
    ...
    directory_path = os.path.dirname(os.path.abspath(sys.argv[0])).replace(":\\", "://").replace("\\", "/")  + "/Dog_Breed_Classification/"    
...
def load_dataset(df_labels, im_size, train_x, train_y, batch_size, image_data, encoder, directory_path, epochs):
    ...
    # Generate image for the testing set
    test_datagen = ImageDataGenerator()
    test_generator = test_datagen.flow(x_test,
                                       y_test,
                                       batch_size=batch_size)

    return create_model(im_size, train_generator, x_train, batch_size, test_generator, x_test, encoder, directory_path, image_data, epochs, y_train)

def create_model(im_size, train_generator, x_train, batch_size, test_generator, x_test, encoder, directory_path, image_data, epochs, y_train):
    ...
    return train_model(model, train_generator, x_train, batch_size, epochs, test_generator, x_test, encoder, directory_path, im_size, image_data)

def train_model(model, train_generator, x_train, batch_size, epochs, test_generator, x_test, encoder, directory_path, im_size, image_data):    
    model.fit(train_generator,
              steps_per_epoch=len(x_train) // batch_size,  # Adjusted steps_per_epoch
              epochs=epochs,
              validation_data=test_generator,
              validation_steps=len(x_test) // batch_size)  # Adjusted validation_steps
